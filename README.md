...
|  Summarizer        GLM-5                      (abstractive summary)       |
|  IOC Extractor     GLM-5                      (IPs, domains, hashes, CVEs)|
|  Entity Extractor  GLM-5                      (actors, orgs, malware)     |
|  Graph Bridge                                 (entity -> graph write)     |
|  Librarian         GLM-5                      (sub-classification)        |
...
Shared data store: PostgreSQL (16 tables, 9 migrations)
...
- **RSS / web feeds** — Polls RSS, Atom, and scrapeable web pages on configurable intervals.
- **Autonomous source discovery** — Extracts URLs from ingested content, resolves them against a configurable domain blacklist, and queues novel sources for operator approval.
...
- **Summarization** — GLM-5 generates abstractive summaries of raw collected content.
- **IOC extraction** — GLM-5 extracts IPs, domains, URLs, file hashes (MD5/SHA-1/SHA-256), CVEs, and email addresses with source context.
- **Entity extraction** — GLM-5 identifies named actors, organizations, malware families, and tools, written to the knowledge graph.
- **Graph bridge** — Connects extracted entities and IOCs with typed relationships across findings.
- **Librarian** — GLM-5 applies fine-grained sub-classification after initial categorization.
...
- **Module registry** — All 22 modules report structured status (running state, throughput, error counts, last activity, AI provider/model) to a shared registry visible from the System Status dashboard page.
...
- Go 1.25+
- PostgreSQL 14+
- API key from [open.bigmodel.cn](https://open.bigmodel.cn) (GLM-5) — required for extraction and summarization
- Groq API key — required for classification (fast path)
...
|---|---|---|
| `NOCTIS_LLM_API_KEY` | Yes | GLM-5 API key (bigmodel.cn) |
| `NOCTIS_GROQ_API_KEY` | Yes | Groq API key (classification) |
| `NOCTIS_GEMINI_API_KEY` | Yes | Gemini API key (brain modules) |
| `NOCTIS_DASHBOARD_API_KEY` | Yes | Dashboard Bearer token |
| `NOCTIS_DB_DSN` | Yes | PostgreSQL connection string |
| `NOCTIS_NVD_API_KEY` | No | NVD API key (higher rate limit) |
| `NOCTIS_ABUSEIPDB_KEY` | No | AbuseIPDB enrichment key |
| `NOCTIS_VT_KEY` | No | VirusTotal enrichment key |
...
noctis:
  ...
  # GLM-5: extraction, summarization, sub-classification
  llm:
    provider: glm
    baseURL: "https://open.bigmodel.cn/api/coding/paas/v4"
    model: "glm-5"
  ...
  # Groq / llama-4-scout: fast classification on the alert path
  llmFast:
    provider: groq
...
The Telegram collector polls the database every 5 minutes and subscribes to newly added channels automatically. Public channels are joined via `ChannelsJoinChannel` — no manual join from the phone app is needed.
...
| System Status | Live status of all 22 modules: running state, throughput, errors, AI provider/model |
...
All 22 modules register with `modules.Registry` and expose a structured `ModuleStatus` with ID, running state, throughput counters, error counts, last activity timestamp, and AI provider/model info. The System Status dashboard page polls `/api/system/status` to display this in real time.

### Module IDs

| Category | Module ID |
|---|---|
| Collector | `collector.telegram`, `collector.rss`, `collector.paste`, `collector.forum` |
| Processor | `processor.classifier`, `processor.summarizer`, `processor.librarian`, `processor.ioc_extractor`, `processor.entity_extractor`, `processor.graph_bridge`, `processor.ioc_lifecycle`, `processor.enrichment` |
| Brain | `brain.correlator`, `brain.analyst`, `brain.brief_generator`, `brain.query_engine` |
| Infrastructure | `infra.dashboard`, `infra.discovery`, `infra.source_analyzer`, `infra.vuln_ingestor` |
...
migrations/              001 through 009 (SQL files, applied in order)
prompts/                 9 LLM prompt templates (classify, classify_detail, extract_iocs,
                           extract_entities, severity, summarize, evaluate_correlation,
                           daily_brief, stylometry)
...
Required secret keys: `NOCTIS_LLM_API_KEY`, `NOCTIS_GROQ_API_KEY`, `NOCTIS_GEMINI_API_KEY`, `NOCTIS_DASHBOARD_API_KEY`, `NOCTIS_DB_PASSWORD`, `NOCTIS_DB_DSN`.
